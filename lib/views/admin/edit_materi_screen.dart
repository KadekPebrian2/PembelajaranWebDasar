...
class _EditMateriScreenState extends State<EditMateriScreen> {
  ...
  void initState() {
    super.initState();
    _judulController =
        TextEditingController(text: widget.materiData?['judul_bab'] ?? '');
    _isiController =
        TextEditingController(text: widget.materiData?['isi_materi'] ?? '');
  }
  ...
  void _saveMateri() async {
    ...
    try {
      // 🛠️ PERBAIKAN LOGIKA: Normalisasi String Kategori khusus JavaScript ke singkatan tunggal "JS"
      String namaKategori = widget.category.trim();
      if (namaKategori.toUpperCase() == "JAVASCRIPT" || namaKategori.toUpperCase() == "JS") {
        namaKategori = "JS";
      } else {
        namaKategori = namaKategori.toUpperCase();
      }
      ...
      if (widget.materiData == null) {
        ...
        List<dynamic> listBabTemp = [];

        if (docSnap.exists) {
          listBabTemp = List.from(docSnap.data()?['daftar_bab'] ?? []);
        }
        listBabTemp.add(mapMateri);
        await docRef.set({
          'kategori': namaKategori,
          'daftar_bab': listBabTemp,
        }, SetOptions(merge: true));
      } else {
        final docSnap = await docRef.get();
        if (docSnap.exists) {
          List<dynamic> listBabTemp =
              List.from(docSnap.data()?['daftar_bab'] ?? []);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Materi sukses disimpan!')));
        Navigator.pop(context);
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Gagal menyimpan materi: $e')));
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _isLoading
          ? const Center(
              child: CircularProgressIndicator(color: Color(0xFF6B11D6)))
          : SafeArea(
              child: Column(
                children: [
                  Expanded(
                            ...
                            if (!isEdit)
                              Container(
                                ...
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    const Text("Modul: Pemrograman Dasar",
                                        style: TextStyle(
                                    ...
                                    const Text(
                                        "Modul ini membahas tentang struktur dasar dokumen web menggunakan bahasa markup terbaru.",
                                        style: TextStyle(
                                    ...
                                    child: Row(
                                      children: const [
                                        ...
                                        Icon(Icons.title_rounded,
                                            color: Color(0xFFFF7A22), size: 20),
                                        SizedBox(width: 16),
