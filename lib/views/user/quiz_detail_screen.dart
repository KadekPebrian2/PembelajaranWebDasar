...
class _QuizDetailScreenState extends State<QuizDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<CourseProvider>(context, listen: false).fetchPaketKuis(widget.kategori);
    });
  }
  ...
  void _nextQuestion(List<dynamic> daftarSoal) {
    if (_selectedAnswer == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: const Text("Silakan pilih salah satu jawaban dulu!"), backgroundColor: widget.temaColor, behavior: SnackBarBehavior.floating),
      );
      return;
    }
    ...
    final jawabanBenar = _getStringAman(
      soalAktif['jawaban_benar'] ?? soalAktif['jawaban'] ?? soalAktif['correct_answer'], 
      ''
    ).trim();
  ...
  void _showResultDialog() {
    ...
    String namaKuis = _getStringAman(_activePaketQuiz?["nama_kuis"] ?? _activePaketQuiz?["id"], "Kuis");
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    style: ElevatedButton.styleFrom(backgroundColor: widget.temaColor, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)), elevation: 0),
                    onPressed: () {
                      Navigator.pop(context);
                      setState(() { _activePaketQuiz = null; _currentIndex = 0; _score = 0; _selectedAnswer = null; });
                    },
                    child: const Text("Selesai", style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.bold)),
                  ),
                ),
              ],
            ),
    ...
    return Consumer<CourseProvider>(
      builder: (context, provider, child) {
        ...
        if (_activePaketQuiz != null && provider.daftarPaketQuiz.isNotEmpty) {
          final matchedQuiz = provider.daftarPaketQuiz.where((paket) => paket['id'] == _activePaketQuiz!['id']).toList();
                ...
                : provider.daftarPaketQuiz.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final String namaKuis = _getStringAman(paket["nama_kuis"] ?? paket["id"], "Kuis");
                          final List<dynamic> soalList = paket["soal_list"] is List ? paket["soal_list"] : [];
                          ...
                          return Container(
                            ...
                            child: Material(
                              ...
                              child: InkWell(
                                ...
                                onTap: () {
                                  if (soalList.isEmpty) {
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Kuis ini belum memiliki pertanyaan.")));
                                    return;
                                  }
                                  setState(() {
                                ...
                                child: Padding(
                                  ...
                                  child: Row(
                                    children: [
                                      Container(
                                        width: 44, height: 44,
                                        decoration: BoxDecoration(color: widget.temaColor.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)),
                                        child: Center(child: Icon(Icons.assignment_rounded, color: widget.temaColor, size: 22)),
                                      ),
        ...
        List<dynamic> listSoal = _activePaketQuiz!["soal_list"] is List ? _activePaketQuiz!["soal_list"] : [];
        ...
        if (soalAktif['opsi'] is List) {
          opsiJawaban = (soalAktif['opsi'] as List).map((e) => e.toString()).toList();
        } else if (soalAktif['pilihan'] is List) {
          opsiJawaban = (soalAktif['pilihan'] as List).map((e) => e.toString()).toList();
        } else {
          if (soalAktif['a'] != null) opsiJawaban.add(soalAktif['a'].toString());
          if (soalAktif['b'] != null) opsiJawaban.add(soalAktif['b'].toString());
          if (soalAktif['c'] != null) opsiJawaban.add(soalAktif['c'].toString());
          if (soalAktif['d'] != null) opsiJawaban.add(soalAktif['d'].toString());
        }
                    ...
                    child: ListView(
                      children: opsiJawaban.map<Widget>((opsiTeks) {
                        ...
                        return GestureDetector(
                          onTap: () => setState(() => _selectedAnswer = opsiTeks),
